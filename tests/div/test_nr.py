...
@pytest.mark.parametrize("mod,clks", [
    (MulticycleDiv(12, impl=Impl.NON_RESTORING), 1.0 / 12e6)
]) 
def test_reference_div(sim, reference_tb):
    sim.run(testbenches=[partial(reference_tb, 15 - 1)])
...
def test_signed_unsigned_mismatch(sim, mismatch_tb):
    sim.run(testbenches=[partial(mismatch_tb, 15 - 1)])
...
@pytest.mark.parametrize("n,d,q,r", [(-(2**31), -1, -(2**31), 0),
                                     ...
                                     (0xff, 0, -1, 0xff)])
@pytest.mark.parametrize("mod,clks", [
    (MulticycleDiv(32, impl=Impl.NON_RESTORING), 1.0 / 12e6)
]) 
def test_riscv_compliance(sim, riscv_tb):
    sim.run(testbenches=[partial(riscv_tb, 35 - 1)])


@pytest.mark.parametrize("mod,clks", [
    (MulticycleDiv(8, impl=Impl.NON_RESTORING), 1.0 / 12e6)
]) 
def test_div_8bit_signed(sim, signed_tb):
    sim.run(testbenches=[partial(signed_tb, 11 - 1)])
...
def test_div_8bit_unsigned(sim, unsigned_tb):
    sim.run(testbenches=[partial(unsigned_tb, 11 - 1)])
